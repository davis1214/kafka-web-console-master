...
app.controller("ZookeepersController", function ($scope, $http, $location) {
    ...
    $scope.createZookeeper = function (zookeeper) {
        $http.put('zookeepers.json', { name: zookeeper.name, host: zookeeper.host, port: zookeeper.port, group: zookeeper.group.name, chroot: zookeeper.chroot, topicConfig: zookeeper.topicConfig}).success(function () {
            $location.path("/");
        });
    };
