RAML.Inspector = function () {
    ...
    function extractResources(basePathSegments, api, securitySchemes) {
        ...
        return apiResources.forEach(function (resource) {
            var resourcePathSegments = basePathSegments.concat(RAML.Client.createPathSegment(resource)), overview = exports.resourceOverviewSource(resourcePathSegments, resource);
            if (overview.methods = overview.methods.map(function (method) {
                return RAML.Inspector.Method.create(method, securitySchemes)
            }), resources.push(overview), resource.resources) {
    ...
    function groupResources(resources) {
        ...
        return(resources || []).forEach(function (resource) {
            ...
            prefix === currentPrefix || 0 === prefix.indexOf(currentPrefix + "/") ? resourceGroups[resourceGroups.length - 1].push(resource) : (currentPrefix = resource.pathSegments[0].toString(), resourceGroups.push([resource]))
        }), resourceGroups
    }

    var exports = {}, METHOD_ORDERING = ["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS", "TRACE", "CONNECT"];
    return exports.resourceOverviewSource = function (pathSegments, resource) {
        var clone = RAML.Utils.clone(resource);
        return clone.traits = resource.is, clone.resourceType = resource.type, clone.type = clone.is = void 0, clone.pathSegments = pathSegments, clone.methods = resource.methods || [], clone.methods.sort(function (a, b) {
            var aOrder = METHOD_ORDERING.indexOf(a.method.toUpperCase()), bOrder = METHOD_ORDERING.indexOf(b.method.toUpperCase());
            return aOrder > bOrder ? 1 : -1
        }), clone.uriParametersForDocumentation = pathSegments.map(function (segment) {
            ...
            for (var key in parameters) {
                ...
                parameter && (parameter = parameter instanceof Array ? parameter : [parameter]), accum[key] = parameter
            }
            return accum
        ...
        return api.baseUri && (api.baseUri = RAML.Client.createBaseUri(api)), api.resources = extractResources([], api, api.securitySchemes), api.resourceGroups = groupResources(api.resources), api
    }, exports
}(), function () {
    "use strict";
    function ensureArray(value) {
        return void 0 !== value && null !== value ? value instanceof Array ? value : [value] : void 0
    }
    ...
    function filterHeaders(headers) {
        var filtered = {plain: {}, parameterized: {}};
        return Object.keys(headers || {}).forEach(function (key) {
            key.match(PARAMETER) ? filtered.parameterized[key] = wrapWithParameterizedHeader(key, headers[key]) : filtered.plain[key] = headers[key]
        }), filtered
    }

    function processBody(body) {
        var content = body["application/x-www-form-urlencoded"];
        content && normalizeNamedParameters(content.formParameters), content = body["multipart/form-data"], content && normalizeNamedParameters(content.formParameters)
    }

    function processResponses(responses) {
        Object.keys(responses).forEach(function (status) {
            var response = responses[status];
            response && normalizeNamedParameters(response.headers)
        })
    }

    function securitySchemesExtractor(securitySchemes) {
        return securitySchemes = securitySchemes || [], function () {
            var securedBy = this.securedBy || [], selectedSchemes = {};
            return securedBy = securedBy.filter(function (name) {
                return null !== name && "object" != typeof name
            }), securitySchemes.forEach(function (scheme) {
                securedBy.forEach(function (name) {
                    scheme[name] && (selectedSchemes[name] = scheme[name])
                })
            }), selectedSchemes
        }
    }
    ...
    var PARAMETER = /\{\*\}/;
    RAML.Inspector.Method = {create: function (raml, securitySchemes) {
        var method = RAML.Utils.clone(raml);
        return method.responseCodes = Object.keys(method.responses || {}), method.securitySchemes = securitySchemesExtractor(securitySchemes), method.allowsAnonymousAccess = allowsAnonymousAccess, normalizeNamedParameters(method.headers), normalizeNamedParameters(method.queryParameters), method.headers = filterHeaders(method.headers), processBody(method.body || {}), processResponses(method.responses || {}), method.plainAndParameterizedHeaders = RAML.Utils.copy(method.headers.plain), Object.keys(method.headers.parameterized).forEach(function (parameterizedHeader) {
    ...
    function validate(value) {
        if (value = value ? value.trim() : "", "" === value)throw new Error;
        return value
    }

    function fromRAML(name, definition) {
        var parameterizedString = new RAML.Client.ParameterizedString(name, definition);
        return{create: function (value) {
            value = validate(value);
            var header = RAML.Utils.clone(definition);
            return header.displayName = parameterizedString.render({"*": value}), header
        }, definition: function () {
            return definition
        }}
    }

    RAML.Inspector.ParameterizedHeader = {fromRAML: fromRAML}
}(), function () {
    function createConfiguration(parsed) {
        var config = {baseUriParameters: {}};
        return{baseUriParameters: function (baseUriParameters) {
            config.baseUriParameters = baseUriParameters || {}
        }, getBaseUri: function () {
            var template = RAML.Client.createBaseUri(parsed);
            return config.baseUriParameters.version = parsed.version, template.render(config.baseUriParameters)
        }}
    }
    ...
    RAML.Client = {create: function (parsed, configure) {
        var configuration = createConfiguration(parsed);
        return configure && configure(configuration), new Client(configuration)
    ...
    RAML.Client.AuthStrategies = {"for": function (scheme, credentials) {
        if (!scheme)return RAML.Client.AuthStrategies.anonymous();
        switch (scheme.type) {
    ...
    RAML.Client.AuthStrategies.Anonymous = Anonymous, RAML.Client.AuthStrategies.anonymous = function () {
        return anonymous
    }
}(), function () {
    ...
    var Oauth1 = function (scheme, credentials) {
        ...
        this.requestTemporaryCredentials = RAML.Client.AuthStrategies.Oauth1.requestTemporaryCredentials(scheme.settings, signerFactory), this.requestAuthorization = RAML.Client.AuthStrategies.Oauth1.requestAuthorization(scheme.settings), this.requestTokenCredentials = RAML.Client.AuthStrategies.Oauth1.requestTokenCredentials(scheme.settings, signerFactory)
    };
    Oauth1.proxyRequest = function (url) {
        return RAML.Settings.proxy && (url = RAML.Settings.proxy + url), url
    }, Oauth1.parseUrlEncodedData = function (data) {
        var result = {};
        return data.split("&").forEach(function (param) {
    ...
    RAML.Client.AuthStrategies.Oauth1.requestAuthorization = function (settings) {
        return function (temporaryCredentials) {
            var authorizationUrl = settings.authorizationUri + "?oauth_token=" + temporaryCredentials.token, deferred = $.Deferred();
            return window.RAML.authorizationSuccess = function (authResult) {
                temporaryCredentials.verifier = authResult.verifier, deferred.resolve(temporaryCredentials)
            }, window.open(authorizationUrl, WINDOW_NAME), deferred.promise()
        }
    }
}(), function () {
    "use strict";
    RAML.Client.AuthStrategies.Oauth1.requestTemporaryCredentials = function (settings, signerFactory) {
        return function () {
            var url = RAML.Client.AuthStrategies.Oauth1.proxyRequest(settings.requestTokenUri), request = RAML.Client.Request.create(url, "post");
            return signerFactory().sign(request), $.ajax(request.toOptions()).then(function (rawFormData) {
                var data = RAML.Client.AuthStrategies.Oauth1.parseUrlEncodedData(rawFormData);
                return{token: data.oauth_token, tokenSecret: data.oauth_token_secret}
            })
        }
    }
}(), function () {
    ...
    RAML.Client.AuthStrategies.Oauth1.requestTokenCredentials = function (settings, signerFactory) {
        return function (temporaryCredentials) {
            var url = RAML.Client.AuthStrategies.Oauth1.proxyRequest(settings.tokenCredentialsUri), request = RAML.Client.Request.create(url, "post");
            return signerFactory(temporaryCredentials).sign(request), $.ajax(request.toOptions()).then(function (rawFormData) {
                var credentials = RAML.Client.AuthStrategies.Oauth1.parseUrlEncodedData(rawFormData);
                return signerFactory({token: credentials.oauth_token, tokenSecret: credentials.oauth_token_secret})
            })
        }
    }
}(), function () {
    ...
    function baseParameters(consumerCredentials) {
        return{oauth_consumer_key: consumerCredentials.consumerKey, oauth_version: "1.0"}
    }

    var Signer = RAML.Client.AuthStrategies.Oauth1.Signer = {};
    Signer.createFactory = function (settings, consumerCredentials) {
        return settings = settings || {}, function (tokenCredentials) {
            var type = "PLAINTEXT" === settings.signatureMethod ? "Plaintext" : "Hmac", mode = void 0 === tokenCredentials ? "Temporary" : "Token";
        ...
        return result.oauth_callback = RAML.Settings.oauth1RedirectUri, result
    }, Signer.generateTokenCredentialParameters = function (consumerCredentials, tokenCredentials) {
        var result = baseParameters(consumerCredentials);
        return result.oauth_token = tokenCredentials.token, tokenCredentials.verifier && (result.oauth_verifier = tokenCredentials.verifier), result
    ...
    function uriWithoutProxy(url) {
        return RAML.Settings.proxy && (url = url.replace(RAML.Settings.proxy, "")), url
    }

    function generateSignature(params, request, key) {
        params.oauth_signature_method = "HMAC-SHA1", params.oauth_timestamp = Math.floor(Date.now() / 1e3), params.oauth_nonce = CryptoJS.lib.WordArray.random(16).toString();
        var data = Hmac.constructHmacText(request, params), hash = CryptoJS.HmacSHA1(data, key);
        params.oauth_signature = hash.toString(CryptoJS.enc.Base64)
    }

    var generateTemporaryCredentialParameters = RAML.Client.AuthStrategies.Oauth1.Signer.generateTemporaryCredentialParameters, generateTokenCredentialParameters = RAML.Client.AuthStrategies.Oauth1.Signer.generateTokenCredentialParameters, rfc3986Encode = RAML.Client.AuthStrategies.Oauth1.Signer.rfc3986Encode, setRequestHeader = RAML.Client.AuthStrategies.Oauth1.Signer.setRequestHeader, Hmac = {constructHmacText: function (request, oauthParams) {
        var options = request.toOptions();
        return[options.type.toUpperCase(), this.encodeURI(options.url), rfc3986Encode(this.encodeParameters(request, oauthParams))].join("&")
    }, encodeURI: function (uri) {
        var parser = document.createElement("a");
        parser.href = uriWithoutProxy(uri);
        var hostname = "";
        return hostname = "https:" === parser.protocol && 443 === parser.port || "http:" === parser.protocol && 80 === parser.port ? parser.hostname.toLowerCase() : parser.host.toLowerCase(), rfc3986Encode(parser.protocol + "//" + hostname + parser.pathname)
    }, encodeParameters: function (request, oauthParameters) {
        var params = request.queryParams(), formParams = {};
        "application/x-www-form-urlencoded" === request.toOptions().contentType && (formParams = request.data());
        var result = [];
        for (var key in params)result.push([rfc3986Encode(key), rfc3986Encode(params[key])]);
        for (var formKey in formParams)result.push([rfc3986Encode(formKey), rfc3986Encode(formParams[formKey])]);
        for (var oauthKey in oauthParameters)result.push([rfc3986Encode(oauthKey), rfc3986Encode(oauthParameters[oauthKey])]);
        return result.sort(function (a, b) {
            return a[0] === b[0] ? a[1].localeCompare(b[1]) : a[0].localeCompare(b[0])
        }), result.map(function (tuple) {
            return tuple.join("=")
        }).join("&")
    }};
    Hmac.Temporary = function (consumerCredentials) {
        ...
        var params = generateTemporaryCredentialParameters(this.consumerCredentials), key = rfc3986Encode(this.consumerCredentials.consumerSecret) + "&";
        generateSignature(params, request, key), setRequestHeader(params, request)
    }, Hmac.Token = function (consumerCredentials, tokenCredentials) {
        this.consumerCredentials = consumerCredentials, this.tokenCredentials = tokenCredentials
    }, Hmac.Token.prototype.sign = function (request) {
        var params = generateTokenCredentialParameters(this.consumerCredentials, this.tokenCredentials), key = rfc3986Encode(this.consumerCredentials.consumerSecret) + "&" + rfc3986Encode(this.tokenCredentials.tokenSecret);
        generateSignature(params, request, key), setRequestHeader(params, request)
    }, RAML.Client.AuthStrategies.Oauth1.Signer.Hmac = Hmac
}(), function () {
    "use strict";
    var generateTemporaryCredentialParameters = RAML.Client.AuthStrategies.Oauth1.Signer.generateTemporaryCredentialParameters, generateTokenCredentialParameters = RAML.Client.AuthStrategies.Oauth1.Signer.generateTokenCredentialParameters, rfc3986Encode = RAML.Client.AuthStrategies.Oauth1.Signer.rfc3986Encode, setRequestHeader = RAML.Client.AuthStrategies.Oauth1.Signer.setRequestHeader, Plaintext = {};
    Plaintext.Temporary = function (consumerCredentials) {
        ...
        params.oauth_signature = rfc3986Encode(this.consumerCredentials.consumerSecret) + "&", params.oauth_signature_method = "PLAINTEXT", setRequestHeader(params, request)
        ...
        params.oauth_signature = rfc3986Encode(this.consumerCredentials.consumerSecret) + "&" + rfc3986Encode(this.tokenCredentials.tokenSecret), params.oauth_signature_method = "PLAINTEXT", setRequestHeader(params, request)
    }, RAML.Client.AuthStrategies.Oauth1.Signer.Plaintext = Plaintext
}(), function () {
    "use strict";
    var Oauth2 = function (scheme, credentials) {
        this.grant = RAML.Client.AuthStrategies.Oauth2.Grant.create(scheme.settings, credentials), this.tokenFactory = RAML.Client.AuthStrategies.Oauth2.Token.createFactory(scheme)
    };
    ...
    RAML.Client.AuthStrategies.Oauth2.credentialsManager = function (credentials, responseType) {
        return{authorizationUrl: function (baseUrl) {
            return baseUrl + "?client_id=" + credentials.clientId + "&response_type=" + responseType + "&redirect_uri=" + RAML.Settings.oauth2RedirectUri
        }, accessTokenParameters: function (code) {
            return{client_id: credentials.clientId, client_secret: credentials.clientSecret, code: code, grant_type: "authorization_code", redirect_uri: RAML.Settings.oauth2RedirectUri}
        }}
    }
}(), function () {
    "use strict";
    function grantTypeFrom(settings) {
        var authorizationGrants = settings.authorizationGrants || [], filtered = authorizationGrants.filter(function (grant) {
            return grant === IMPLICIT_GRANT
        }), specifiedGrant = filtered[0] || authorizationGrants[0];
        if (!GRANTS.some(function (grant) {
            return grant === specifiedGrant
        }))throw new Error("Unknown grant type: " + specifiedGrant);
        return specifiedGrant
    }

    var GRANTS = ["code", "token"], IMPLICIT_GRANT = "token", Oauth2 = RAML.Client.AuthStrategies.Oauth2, Grant = {create: function (settings, credentials) {
        var type = grantTypeFrom(settings), credentialsManager = Oauth2.credentialsManager(credentials, type), className = type.charAt(0).toUpperCase() + type.slice(1);
        return new this[className](settings, credentialsManager)
    }};
    Grant.Code = function (settings, credentialsManager) {
        this.settings = settings, this.credentialsManager = credentialsManager
    }, Grant.Code.prototype.request = function () {
        var requestAuthorization = Oauth2.requestAuthorization(this.settings, this.credentialsManager), requestAccessToken = Oauth2.requestAccessToken(this.settings, this.credentialsManager);
    ...
    function accessTokenFromString(data) {
        for (var vars = data.split("&"), i = 0; i < vars.length; i++) {
            var pair = vars[i].split("=");
            if ("access_token" === decodeURIComponent(pair[0]))return decodeURIComponent(pair[1])
        }
        return void 0
    }
    ...
    RAML.Client.AuthStrategies.Oauth2.requestAccessToken = function (settings, credentialsManager) {
        return function (code) {
            var url = proxyRequest(settings.accessTokenUri), requestOptions = {url: url, type: "post", data: credentialsManager.accessTokenParameters(code)};
            return $.ajax(requestOptions).then(function (data) {
                var extract = accessTokenFromString;
                return"object" == typeof data && (extract = accessTokenFromObject), extract(data)
            })
        }
    }
}(), function () {
    ...
    RAML.Client.AuthStrategies.Oauth2.requestAuthorization = function (settings, credentialsManager) {
        var authorizationUrl = credentialsManager.authorizationUrl(settings.authorizationUri), deferred = $.Deferred();
        return window.RAML.authorizationSuccess = function (code) {
    ...
    function tokenConstructorFor(scheme) {
        var describedBy = scheme.describedBy || {}, headers = describedBy.headers || {}, queryParameters = describedBy.queryParameters || {};
        return headers.Authorization ? Header : queryParameters.access_token ? QueryParameter : Header
    }
    ...
    }, RAML.Client.AuthStrategies.Oauth2.Token = {createFactory: function (scheme) {
    ...
    function rendererFor(template, uriParameters) {
        ...
        return function (context) {
            context = context || {}, requiredParameters.forEach(function (name) {
                if (!context[name])throw new Error("Missing required uri parameter: " + name)
            });
    ...
    RAML.Client.ParameterizedString = function (template, uriParameters, options) {
        options = options || {parameterValues: {}}, template = template.replace(templateMatcher, function (match, parameterName) {
            return options.parameterValues[parameterName] ? options.parameterValues[parameterName] : "{" + parameterName + "}"
        }), this.parameters = uriParameters, this.templated = Object.keys(this.parameters || {}).length > 0, this.tokens = tokenize(template), this.render = rendererFor(template, uriParameters), this.toString = function () {
            return template
        }
    }
}(), function () {
    "use strict";
    RAML.Client.PathBuilder = {create: function (pathSegments) {
        return function (contexts) {
            return contexts = contexts || [], pathSegments.map(function (pathSegment, index) {
    ...
    "use strict";
    var CONTENT_TYPE = "content-type", FORM_DATA = "multipart/form-data", RequestDsl = function (options) {
        var rawData, queryParams, isMultipartRequest;
        this.data = function (data) {
            return void 0 === data ? RAML.Utils.clone(rawData) : void(rawData = data)
        }, this.queryParams = function (parameters) {
            return void 0 === parameters ? RAML.Utils.clone(queryParams) : void(queryParams = parameters)
        }, this.queryParam = function (name, value) {
            queryParams = queryParams || {}, queryParams[name] = value
        }, this.header = function (name, value) {
            if (options.headers = options.headers || {}, name.toLowerCase() === CONTENT_TYPE) {
                if (value === FORM_DATA)return void(isMultipartRequest = !0);
                isMultipartRequest = !1, options.contentType = value
            }
            options.headers[name] = value
        }, this.headers = function (headers) {
            options.headers = {}, isMultipartRequest = !1, options.contentType = !1;
            for (var name in headers)this.header(name, headers[name])
        }, this.toOptions = function () {
            var o = RAML.Utils.clone(options);
            if (o.traditional = !0, rawData)if (isMultipartRequest) {
                var data = new FormData, appendValueForKey = function (key) {
                    return function (value) {
                        data.append(key, value)
                    }
                };
                for (var key in rawData)rawData[key].forEach(appendValueForKey(key));
                o.processData = !1, o.data = data
            } else o.processData = !0, o.data = rawData;
            if (!RAML.Utils.isEmpty(queryParams)) {
                var separator = options.url.match("\\?") ? "&" : "?";
                o.url = options.url + separator + $.param(queryParams, !0)
            }
            return o
        }
    };
    RAML.Client.Request = {create: function (url, method) {
        var request = {};
        return RequestDsl.call(request, {url: url, type: method, contentType: !1}), request
    }}
}(), function () {
    "use strict";
    ...
    function baseValidations(definition) {
        var validations = {};
        return definition.required && (validations.required = VALIDATIONS.required), validations
    }

    function numberValidations(validations, definition) {
        definition.minimum && (validations.minimum = VALIDATIONS.minimum(definition.minimum)), definition.maximum && (validations.maximum = VALIDATIONS.maximum(definition.maximum))
    }
    ...
    var RFC1123 = /^(Mon|Tue|Wed|Thu|Fri|Sat|Sun), \d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4} \d{2}:\d{2}:\d{2} GMT$/, VALIDATIONS = {required: function (value) {
        return!isEmpty(value)
    }, "boolean": function (value) {
        return isEmpty(value) || "true" === value || "false" === value
    }, "enum": function (enumeration) {
        return function (value) {
            return isEmpty(value) || enumeration.some(function (item) {
                return item === value
            })
        }
    }, integer: function (value) {
        return isEmpty(value) || !!/^-?(0|[1-9][0-9]*)$/.exec(value)
    }, number: function (value) {
        return isEmpty(value) || !!/^-?(0|[1-9][0-9]*)(\.[0-9]*)?([eE][-+]?[0-9]+)?$/.exec(value)
        ...
        return function (value) {
            return isEmpty(value) || maximum >= value
        }
        ...
        return isEmpty(value) || !!RFC1123.exec(value)
    }}, VALIDATIONS_FOR_TYPE = {string: function (definition) {
        var validations = baseValidations(definition);
        return definition.enum && (validations.enum = VALIDATIONS.enum(definition.enum)), definition.minLength && (validations.minLength = VALIDATIONS.minLength(definition.minLength)), definition.maxLength && (validations.maxLength = VALIDATIONS.maxLength(definition.maxLength)), definition.pattern && (validations.pattern = VALIDATIONS.pattern(definition.pattern)), validations
    }, integer: function (definition) {
        var validations = baseValidations(definition);
        return validations.integer = VALIDATIONS.integer, numberValidations(validations, definition), validations
    }, number: function (definition) {
        var validations = baseValidations(definition);
        return validations.number = VALIDATIONS.number, numberValidations(validations, definition), validations
    }, "boolean": function (definition) {
        var validations = baseValidations(definition);
        return validations.boolean = VALIDATIONS.boolean, validations
    }, date: function (definition) {
        var validations = baseValidations(definition);
        return validations.date = VALIDATIONS.date, validations
    }};
    Validator.prototype.validate = function (value) {
        var errors;
        for (var validation in this.validations)this.validations[validation](value) || (errors = errors || [], errors.push(validation));
        return errors
    }, Validator.from = function (definition) {
        if (!definition)throw new Error("definition is required!");
        var validations;
        return validations = VALIDATIONS_FOR_TYPE[definition.type] ? VALIDATIONS_FOR_TYPE[definition.type](definition) : {}, new Validator(validations)
    }, RAML.Client.Validator = Validator
}(), function () {
    RAML.Controllers = {}
}(), function () {
    "use strict";
    RAML.Controllers.BodyDocumentation = function ($scope, DataStore) {
        $scope.bodyKey = $scope.keyBase + ":body", $scope.displayed = {}, $scope.expandSchema = function (contentType) {
            var key = $scope.bodyKey + ":schemaExpanded:" + contentType;
            return DataStore.set(key, !0)
    ...
    var FORM_MIME_TYPES = ["application/x-www-form-urlencoded", "multipart/form-data"], controller = function ($scope) {
        ...
        function hasParameters() {
            return!(!hasUriParameters() && !$scope.method.queryParameters && RAML.Utils.isEmpty($scope.method.headers.plain) && !hasFormParameters($scope.method))
        }
        ...
        $scope.documentation = this, $scope.generateKey = function (base, method) {
    ...
    var controller = function ($scope, $attrs, ramlParserWrapper) {
        $scope.ramlConsole = this, $attrs.hasOwnProperty("withRootDocumentation") && (this.withRootDocumentation = !0), $scope.src && ramlParserWrapper.load($scope.src), this.keychain = {}
    };
    ...
    RAML.Controllers.Resource = function ($scope, DataStore, $element) {
        $scope.resourceView = this, $scope.$emit("console:resource:rendered", $scope.resource, $element), $scope.$on("$destroy", function () {
            $scope.$emit("console:resource:destroyed", $scope.resource)
        }), this.expanded = DataStore.get(generateKey($scope.resource)), this.openDocumentation = function ($event, method) {
            $event.stopPropagation(), this.expanded || this.toggleExpansion(), $scope.$emit("console:expand", $scope.resource, method, $element)
        }, this.toggleExpansion = function () {
            this.expanded = !this.expanded, DataStore.set(generateKey($scope.resource), this.expanded)
        }
    }
}(), function () {
    function Controller($scope, DataStore) {
        this.tabs = [], this.DataStore = DataStore, this.key = $scope.keyBase + ":tabset", $scope.tabset = this
    }

    Controller.prototype.select = function (tab, dontPersist) {
        tab.disabled || (dontPersist || this.DataStore.set(this.key, tab.heading), this.tabs.forEach(function (item) {
            item.active = !1
        }), tab.active = !0, this.active = tab)
    }, Controller.prototype.addTab = function (tab) {
        var previouslyEnabled = this.DataStore.get(this.key) === tab.heading, allOthersDisabled = this.tabs.every(function (tab) {
            return tab.disabled
        });
        (allOthersDisabled || previouslyEnabled) && this.select(tab, this.DataStore.get(this.key)), this.tabs.push(tab)
    }, RAML.Controllers.tabset = Controller
}(), function () {
    var controller = function ($scope, DataStore) {
        this.DataStore = DataStore, this.key = $scope.keyBase + ":toggle", this.toggleItems = $scope.toggleItems = [], this.toggleModel = $scope.toggleModel || {}, $scope.toggle = this
    };
    controller.prototype.select = function (toggleItem, dontPersist) {
        this.toggleModel.selected = void 0, this.toggleItems.forEach(function (toggleItem) {
            toggleItem.active = !1
        }), toggleItem.active = !0, this.toggleModel.selected = toggleItem.heading, dontPersist || this.DataStore.set(this.key, toggleItem.heading)
    }, controller.prototype.addToggleItem = function (toggleItem) {
        var previouslyEnabled = this.DataStore.get(this.key) === toggleItem.heading, noneActive = this.toggleItems.every(function (toggleItem) {
            return!toggleItem.active
        });
        (noneActive || previouslyEnabled) && this.select(toggleItem, !0), this.toggleItems.push(toggleItem)
    }, RAML.Controllers.toggle = controller
}(), function () {
    function parseHeaders(headers) {
        var key, val, i, parsed = {};
        return headers ? (headers.split("\n").forEach(function (line) {
            i = line.indexOf(":"), key = line.substr(0, i).trim().toLowerCase(), val = line.substr(i + 1).trim(), key && (parsed[key] ? parsed[key] += ", " + val : parsed[key] = val)
        }), parsed) : parsed
    }

    var apply, TryIt = function ($scope, DataStore) {
        $scope.apiClient = this;
        var baseKey = $scope.resource.toString() + ":" + $scope.method.method;
        $scope.baseKey = function () {
            return baseKey
        };
        var contextKey = baseKey + ":context", responseKey = baseKey + ":response", context = new RAML.Controllers.TryIt.Context($scope.resource, $scope.method), oldContext = DataStore.get(contextKey);
        oldContext && context.merge(oldContext), this.context = $scope.context = context, this.response = DataStore.get(responseKey), DataStore.set(contextKey, this.context), this.method = $scope.method, this.httpMethod = $scope.method.method, this.parsed = $scope.api, this.securitySchemes = $scope.method.securitySchemes(), this.keychain = $scope.ramlConsole.keychain, apply = function () {
            $scope.$apply.apply($scope, arguments)
        }, this.setResponse = function (response) {
            return DataStore.set(responseKey, response), $scope.apiClient.response = response, response
        }
    };
    TryIt.prototype.inProgress = function () {
        ...
        function handleResponse(jqXhr) {
            response.body = jqXhr.responseText, response.status = jqXhr.status, response.headers = parseHeaders(jqXhr.getAllResponseHeaders()), response.headers["content-type"] && (response.contentType = response.headers["content-type"].split(";")[0]), apply()
        }

        this.missingUriParameters = !1, this.disallowedAnonymousRequest = !1;
        var url, response = this.setResponse({});
        try {
            var pathBuilder = this.context.pathBuilder, client = RAML.Client.create(this.parsed, function (client) {
            ...
            return this.setResponse(void 0), void(this.missingUriParameters = !0)
        }
        RAML.Settings.proxy && (url = RAML.Settings.proxy + url);
        var request = RAML.Client.Request.create(url, this.httpMethod);
        RAML.Utils.isEmpty(this.context.queryParameters.data()) || request.queryParams(this.context.queryParameters.data()), RAML.Utils.isEmpty(this.context.headers.data()) || request.headers(this.context.headers.data()), this.context.bodyContent && (request.header("Content-Type", this.context.bodyContent.selected), request.data(this.context.bodyContent.data()));
        var authStrategy;
        try {
            "Anonymous" !== this.keychain.selected || this.method.allowsAnonymousAccess() || (this.disallowedAnonymousRequest = !0);
            var scheme = this.securitySchemes && this.securitySchemes[this.keychain.selected], credentials = this.keychain[this.keychain.selected];
            authStrategy = RAML.Client.AuthStrategies.for(scheme, credentials)
        } catch (e) {
        }
        authStrategy.authenticate().then(function (token) {
            token.sign(request), $.ajax(request.toOptions()).then(function (data, textStatus, jqXhr) {
    ...
    "use strict";
    var FORM_URLENCODED = "application/x-www-form-urlencoded", FORM_DATA = "multipart/form-data", BodyContent = function (contentTypes) {
        this.contentTypes = Object.keys(contentTypes), this.selected = this.contentTypes[0];
    ...
    BodyContent.prototype.isForm = function (contentType) {
        ...
        $event.preventDefault(), this.definitions[this.selected].fillWithExample()
        ...
        return this.selected ? this.definitions[this.selected].data() : void 0
    }, BodyContent.prototype.copyFrom = function (oldContent) {
        var content = this;
        oldContent.contentTypes.forEach(function (contentType) {
            content.definitions[contentType] && content.definitions[contentType].copyFrom(oldContent.definitions[contentType])
        }), this.contentTypes.some(function (contentType) {
    ...
    var BodyType = function (contentType) {
        this.contentType = contentType || {}, this.value = void 0
    };
    ...
    var Context = function (resource, method) {
        this.headers = new RAML.Controllers.TryIt.NamedParameters(method.headers.plain, method.headers.parameterized), this.queryParameters = new RAML.Controllers.TryIt.NamedParameters(method.queryParameters), method.body && (this.bodyContent = new RAML.Controllers.TryIt.BodyContent(method.body)), this.pathBuilder = new RAML.Client.PathBuilder.create(resource.pathSegments), this.pathBuilder.baseUriContext = {}, this.pathBuilder.segmentContexts = resource.pathSegments.map(function () {
            return{}
        })
    };
    Context.prototype.merge = function (oldContext) {
        this.headers.copyFrom(oldContext.headers), this.queryParameters.copyFrom(oldContext.queryParameters), this.bodyContent && oldContext.bodyContent && this.bodyContent.copyFrom(oldContext.bodyContent), this.pathBuilder.baseUriContext = oldContext.pathBuilder.baseUriContext, this.pathBuilder.segmentContexts = oldContext.pathBuilder.segmentContexts
    }, RAML.Controllers.TryIt.Context = Context
}(), function () {
    "use strict";
    var NamedParameter = function (definitions) {
        this.definitions = definitions, this.selected = definitions[0].type
    };
    ...
    function filterEmpty(object) {
        ...
        return Object.keys(object).forEach(function (key) {
            var values = object[key].filter(function (value) {
                return void 0 !== value && null !== value && ("string" != typeof value || value.trim().length > 0)
            });
            values.length > 0 && (copy[key] = values)
        }), copy
    }

    var NamedParameters = function (plain, parameterized) {
        this.plain = copy(plain), this.parameterized = parameterized, Object.keys(parameterized || {}).forEach(function (key) {
            parameterized[key].created = []
        }), this.values = {}, Object.keys(this.plain).forEach(function (key) {
            this.values[key] = [void 0]
        }.bind(this))
    };
    NamedParameters.prototype.create = function (name, value) {
        var parameters = this.parameterized[name], definition = parameters.map(function (parameterizedHeader) {
            return parameterizedHeader.create(value)
        }), parameterizedName = definition[0].displayName;
        parameters.created.push(parameterizedName), this.plain[parameterizedName] = new RAML.Controllers.TryIt.NamedParameter(definition), this.values[parameterizedName] = [void 0]
    }, NamedParameters.prototype.remove = function (name) {
        delete this.plain[name], delete this.values[name]
        ...
        Object.keys(oldParameters.parameterized || {}).forEach(function (key) {
            parameters.parameterized[key] && oldParameters.parameterized[key].created.forEach(function (createdParam) {
    ...
    RAML.Directives.apiResources = function () {
        var controller = function ($scope) {
            ...
            this.collapsed = {}, this.toggleAll = function (collapsed) {
        ...
        return controller.prototype.keyFor = function (group) {
            return group[0].pathSegments[0].toString()
        }, {restrict: "E", templateUrl: "views/api_resources.tmpl.html", replace: !0, controller: controller}
    }
}(), function () {
    RAML.Directives.basicAuth = function () {
        return{restrict: "E", templateUrl: "views/basic_auth.tmpl.html", replace: !0, scope: {credentials: "="}}
    }
}(), function () {
    "use strict";
    RAML.Directives.bodyContent = function () {
        return{restrict: "E", templateUrl: "views/body_content.tmpl.html", replace: !0, scope: {body: "="}}
    }
}(), function () {
    "use strict";
    RAML.Directives.bodyDocumentation = function () {
        return{restrict: "E", replace: !0, templateUrl: "views/body_documentation.tmpl.html", scope: {body: "=", keyBase: "="}, controller: RAML.Controllers.BodyDocumentation}
    }
}(), function () {
    "use strict";
    function sanitize(options) {
        var code = options.code || "", formatter = formatters[options.mode] || formatters.default;
        try {
    ...
    var formatters = {"application/json": function (code) {
        ...
        sanitize($scope), this.editor = new CodeMirror($element[0], {mode: $scope.mode, readOnly: !0, value: $scope.code, lineNumbers: !0, indentUnit: 4}), this.editor.setSize("100%", "100%")
    };
    Controller.prototype.refresh = function (options) {
        sanitize(options), this.editor.setOption("mode", options.mode), this.editor.setValue(options.code), this.editor.refresh()
    };
    var link = function (scope, element, attrs, editor) {
        ...
        scope.$watch(watchCode, function (visible) {
            visible && editor.refresh(scope)
        })
    };
    RAML.Directives.codeMirror = function () {
        return{link: link, restrict: "A", replace: !0, controller: Controller, scope: {code: "=codeMirror", visible: "=", mode: "@?"}}
    ...
    RAML.Directives.collapsible = function ($parse) {
        return{controller: Controller, restrict: "EA", scope: !0, link: function (scope, element, attrs) {
            if (!attrs.collapsed && !attrs.expanded)return void(scope.collapsed = !0);
            var attr = attrs.collapsed || attrs.expanded, normalizeForAttribute = function (arg) {
                return attrs.expanded ? !arg : arg
            };
            scope.collapsed = normalizeForAttribute($parse(attr)(scope)), scope.$watch(attr, function (value) {
                scope.collapsed = normalizeForAttribute(value)
            }), scope.$watch("collapsed", function (collapsed) {
                $parse(attr).assign(scope.$parent, normalizeForAttribute(collapsed)), element.removeClass("collapsed expanded"), element.addClass(collapsed ? "collapsed" : "expanded")
            })
        }}
    }, RAML.Directives.collapsibleToggle = function () {
        return{require: "^collapsible", restrict: "EA", link: function (scope, element, attrs, controller) {
            element.bind("click", function () {
                scope.$apply(controller.toggle)
            })
        }}
    }, RAML.Directives.collapsibleContent = function () {
        return{require: "^collapsible", restrict: "EA", link: function (scope, element) {
            scope.$watch("collapsed", function (collapsed) {
    ...
    RAML.Directives.documentation = function () {
        return{controller: RAML.Controllers.Documentation, restrict: "E", templateUrl: "views/documentation.tmpl.html", replace: !0}
    }
}(), function () {
    "use strict";
    RAML.Directives.enum = function ($timeout, $filter) {
        function correctHeight(el, container) {
            var enumRect = el.getBoundingClientRect(), containerRect = container.getBoundingClientRect(), top = enumRect.top, bottom = enumRect.bottom;
            return top <= containerRect.top && (top = containerRect.top), bottom >= containerRect.bottom && (bottom = containerRect.bottom), bottom - top
        }

        var KEY_DOWN = 40, KEY_UP = 38, KEY_ENTER = 13, link = function ($scope, $el) {
            function filterEnumElements() {
                $scope.filteredEnum = $filter("filter")($scope.options, $scope.$parent.model)
            }

            $scope.$watch("$parent.model", filterEnumElements), $scope.selectItem = function (item) {
                $scope.model = $scope.$parent.model = item, $scope.focused = !1
            }, filterEnumElements(), $el.find("input").bind("focus", function () {
                $scope.$apply(function () {
                    $scope.selectedIndex = -1, $scope.focused = !0
                })
            }), $el.find("input").bind("blur", function () {
                $scope.$apply(function () {
                    $scope.model = $scope.$parent.model, $scope.focused = !1
                })
            }), $el.bind("mousedown", function (event) {
                "LI" === event.target.tagName && event.preventDefault()
            }), $el.find("input").bind("input", function () {
                $scope.$apply(function () {
                    $scope.focused = !0, $scope.selectedIndex = 0
                })
            }), $el.find("input").bind("keydown", function (e) {
                switch (e.keyCode) {
                    case KEY_UP:
                        $scope.selectedIndex = $scope.selectedIndex - 1, $scope.selectedIndex = Math.max(0, $scope.selectedIndex), e.preventDefault();
                        break;
                    case KEY_DOWN:
                        $scope.selectedIndex = $scope.selectedIndex + 1, $scope.selectedIndex = Math.min($scope.filteredEnum.length - 1, $scope.selectedIndex), e.preventDefault();
                        break;
                    case KEY_ENTER:
                        var selection = $scope.filteredEnum[$scope.selectedIndex];
                        selection && $scope.selectItem(selection), e.preventDefault()
                }
                $scope.$apply()
            }), $scope.$watch("focused", function () {
                $scope.filteredEnum = $scope.options, setTimeout(function () {
                    var ul = $el.find("ul"), container = $el[0].offsetParent;
                    ul.css("max-height", null), $scope.containedBy && (container = document.querySelector($scope.containedBy)), container && (ul.css("max-height", correctHeight(ul[0], container) + "px"), filterEnumElements(), $scope.$digest())
                })
            })
        };
        return{restrict: "E", transclude: !0, link: link, templateUrl: "views/enum.tmpl.html", scope: {options: "=", model: "=", containedBy: "="}}
    }
}(), function (angular) {
    "use strict";
    ...
    inputOverride.config(["$provide", function ($provide) {
        $provide.decorator("inputDirective", ["$delegate", function ($delegate) {
            return angular.forEach($delegate, function (inputDirective) {
                var originalCompile = inputDirective.compile;
                inputDirective.compile = function (element, attrs) {
                    return attrs.type && "file" === attrs.type.toLowerCase() ? function (scope, element, attr, ctrl) {
                        element.bind("change", function () {
                            scope.$apply(function () {
                                var files = element[0].files, viewValue = attr.multiple ? files : files[0];
    ...
    RAML.Directives.markdown = function ($sanitize) {
        var converter = new Showdown.converter({extensions: ["table"]}), link = function (scope, element) {
        ...
        return{restrict: "A", link: link, scope: {markdown: "="}}
    }
}(), function () {
    "use strict";
    RAML.Directives.method = function () {
        return{restrict: "E", templateUrl: "views/method.tmpl.html", replace: !0}
    }
}(), function () {
    var Controller = function ($scope) {
        $scope.displayParameters = function () {
            var parameters = $scope.parameters || {};
            return parameters.plain = parameters.plain || {}, parameters.parameterized = parameters.parameterized || {}, Object.keys(parameters.plain).length > 0 || Object.keys(parameters.parameterized).length > 0
        }
    };
    RAML.Directives.namedParameters = function () {
        return{restrict: "E", controller: Controller, templateUrl: "views/named_parameters.tmpl.html", replace: !0, scope: {heading: "@", parameters: "="}}
    }
}(), function () {
    "use strict";
    RAML.Directives.namedParametersDocumentation = function () {
        return{restrict: "E", replace: !0, controller: RAML.Controllers.NamedParametersDocumentation, templateUrl: "views/named_parameters_documentation.tmpl.html", scope: {heading: "@", parameters: "="}}
    }
}(), function () {
    RAML.Directives.oauth1 = function () {
        return{restrict: "E", templateUrl: "views/oauth1.tmpl.html", replace: !0, scope: {credentials: "="}}
    }
}(), function () {
    RAML.Directives.oauth2 = function () {
        return{restrict: "E", templateUrl: "views/oauth2.tmpl.html", replace: !0, scope: {credentials: "="}}
    }
}(), function () {
    var Controller = function ($scope) {
        $scope.placeholder = $scope.placeholder || $scope.definition.example, $scope.inputType = "file" === $scope.definition.type ? "file" : $scope.definition.enum ? "enum" : "default"
    };
    RAML.Directives.parameterField = function () {
        return{restrict: "E", templateUrl: "views/parameter_field.tmpl.html", controller: Controller, scope: {name: "=", model: "=", definition: "=", placeholder: "=?", invalidClass: "@?", containedBy: "@"}}
    }
}(), function () {
    RAML.Directives.parameterFields = function () {
        return{restrict: "E", templateUrl: "views/parameter_fields.tmpl.html", scope: {parameters: "="}}
    }
}(), function () {
    "use strict";
    var Controller = function ($scope) {
        $scope.parameterFactory = this, this.parameterName = $scope.parameterName, this.parameters = $scope.parameters
    };
    Controller.prototype.open = function ($event) {
        $event.preventDefault(), this.opened = !0
    }, Controller.prototype.create = function ($event) {
        $event.preventDefault();
        try {
            this.parameters.create(this.parameterName, this.value), this.opened = !1, this.value = this.status = ""
        } catch (e) {
            this.status = "error"
        }
    }, RAML.Directives.parameterizedParameter = function () {
        return{restrict: "E", templateUrl: "views/parameterized_parameter.tmpl.html", replace: !0, controller: Controller, scope: {parameters: "=", parameterName: "="}}
    }
}(), function () {
    "use strict";
    RAML.Directives.ramlConsole = function (ramlParserWrapper, DataStore, $timeout) {
        var link = function ($scope, $el, $attrs, controller) {
            $scope.popOverOpen = function () {
                ...
                $scope.api = controller.api = RAML.Inspector.create(raml), $timeout(function () {
        ...
        return{restrict: "E", templateUrl: "views/raml-console.tmpl.html", controller: RAML.Controllers.RAMLConsole, scope: {src: "@"}, link: link}
    }
}(), function () {
    "use strict";
    RAML.Directives.ramlConsoleInitializer = function (ramlParserWrapper) {
        ...
        controller.prototype.load = function () {
            ramlParserWrapper.load(this.location), this.finished = !0
        }, controller.prototype.parse = function () {
            ramlParserWrapper.parse(this.raml), this.finished = !0
        };
        var link = function ($scope, $element, $attrs, controller) {
            -1 !== document.location.search.indexOf("?raml=") && (controller.location = document.location.search.replace("?raml=", ""), controller.load())
        };
    ...
    RAML.Directives.resource = function () {
        return{restrict: "E", templateUrl: "views/resource.tmpl.html", replace: !0, controller: RAML.Controllers.Resource}
    }
}(), function () {
    "use strict";
    function calculateContainerPosition(container, consoleContainer, rect) {
        container.style.top = consoleContainer.scrollTop + rect.top - consoleContainer.offsetTop + "px", container.style.bottom = consoleContainer.scrollTop + rect.bottom + "px", container.style.height = rect.bottom - rect.top + "px"
    }

    function calculateContainerHeight(container, consoleContainer) {
        container.css("height", consoleContainer[0].clientHeight - 10 + "px"), container[0].style.top = consoleContainer[0].scrollTop + 5 + "px"
    }

    function createPopover(element) {
        var rect, consoleContainer = angular.element(document.body).find("raml-console").parent(), resourceList = angular.element(document.getElementById("raml-console")), placeholder = angular.element(element[0].querySelector(".resource-placeholder")), container = angular.element(element[0].querySelector(".resource-container"));
        return{open: function ($scope, $resourceEl, resource, method) {
            $scope.resource = resource, consoleContainer.css("overflow", "hidden"), placeholder.css("height", resourceList[0].scrollHeight + "px"), container.addClass("grow-expansion-animation"), setTimeout(function () {
                rect = $resourceEl[0].getBoundingClientRect(), calculateContainerPosition(container[0], consoleContainer[0], rect), setTimeout(function () {
                    placeholder.addClass("masked"), calculateContainerHeight(container, consoleContainer), $scope.selectedMethod = method, $scope.$digest()
                })
            })
        }, close: function ($scope) {
            calculateContainerPosition(container[0], consoleContainer[0], rect), setTimeout(function () {
                placeholder.removeClass("masked"), setTimeout(function () {
                    container.removeClass("grow-expansion-animation"), consoleContainer.css("overflow", "auto"), $scope.$apply("resource = undefined"), $scope.$apply("selectedMethod = undefined")
                }, 200)
            })
        }, resize: function () {
    ...
    RAML.Directives.resourceDocumentation = function ($rootScope, $window, DataStore) {
        function prepare($scope, $element, $resourceEl, resource, method) {
            $scope.selectMethod = function (method) {
                DataStore.set(resource.toString() + ":method", method.method), $scope.selectedMethod = method, $scope.keyBase = resource.toString() + ":" + method.method
            }, $scope.closePopover = function (e) {
                e.preventDefault(), DataStore.set(resource.toString() + ":method", void 0), popover.close($scope), popover = void 0
            }, popover = createPopover($element), popover.open($scope, $resourceEl, resource, method), $scope.selectMethod(method)
        }

        function Controller($scope, $element) {
            var receipt;
            $rootScope.$on("console:resource:destroyed", function (event, resource) {
                $scope.resource && $scope.resource.toString() === resource.toString() && (receipt = setTimeout(function () {
                    popover.close($scope), popover = void 0
                }, 100))
                ...
                if (methodName) {
                    ...
                    method && (receipt && $scope.resource && $scope.resource.toString() === resource.toString() ? (clearTimeout(receipt), $scope.resource = resource, $scope.selectedMethod = method) : prepare($scope, $element, $resourceEl, resource, method))
                }
        ...
        return angular.element($window).bind("resize", function () {
            popover && popover.resize()
        }), {restrict: "E", templateUrl: "views/resource_documentation.tmpl.html", controller: Controller, scope: {api: "=", ramlConsole: "="}}
    }
}(), function () {
    "use strict";
    RAML.Directives.repeatable = function ($parse) {
        ...
        return{restrict: "EA", templateUrl: "views/repeatable.tmpl.html", transclude: !0, controller: controller, link: function (scope, element, attrs) {
            scope.repeatable = !attrs.repeatable || $parse(attrs.repeatable)(scope), scope.repeatableModel = $parse(attrs.repeatableModel)(scope), scope.$watch("repeatableModel", function (value) {
    ...
    RAML.Directives.repeatableAdd = function () {
        return{require: "^repeatable", restrict: "E", template: '<i class="fa fa-plus-square" ng-show="visible" ng-click="new()"></i>', scope: !0, link: function (scope, element, attrs, controller) {
    ...
    RAML.Directives.repeatableRemove = function () {
        return{require: "^repeatable", restrict: "E", template: '<i class="fa fa-times-circle" ng-show="visible" ng-click="remove()"></i>', scope: !0, link: function (scope, element, attrs, controller) {
    ...
    RAML.Directives.responses = function () {
        return{restrict: "E", templateUrl: "views/responses.tmpl.html"}
    }
}(), function () {
    "use strict";
    RAML.Directives.rootDocumentation = function () {
        return{restrict: "E", templateUrl: "views/root_documentation.tmpl.html", replace: !0}
    }
}(), function () {
    RAML.Directives.securitySchemes = function () {
        ...
        return controller.prototype.supports = function (scheme) {
            return"OAuth 2.0" === scheme.type || "OAuth 1.0" === scheme.type || "Basic Authentication" === scheme.type
        }, {restrict: "E", templateUrl: "views/security_schemes.tmpl.html", replace: !0, controller: controller, scope: {schemes: "=", keychain: "=", baseKey: "="}}
    }
}(), function () {
    "use strict";
    !function () {
        RAML.Directives.consoleTabset = function () {
            return{restrict: "E", templateUrl: "views/tabset.tmpl.html", replace: !0, transclude: !0, controller: RAML.Controllers.tabset, scope: {heading: "@", keyBase: "@"}}
        }
    }(), function () {
        function Controller($scope) {
            this.registerSubtabs = function (subtabs, keyBase) {
                $scope.subtabs = subtabs, $scope.keyBase = keyBase
        ...
        RAML.Directives.consoleTab = function ($location, $anchorScroll, DataStore) {
            return{restrict: "E", templateUrl: "views/tab.tmpl.html", replace: !0, transclude: !0, require: "^consoleTabset", controller: Controller, link: function ($scope, $element, $attrs, tabsetCtrl) {
                var selected = DataStore.get($scope.keyBase);
                $scope.select = function (subItem) {
                    selected = subItem;
                    var responseCode = $("#" + selected)[0], container = $element.parent()[0];
                    container.scrollTop = responseCode.offsetTop - container.getBoundingClientRect().top + responseCode.offsetParent.getBoundingClientRect().top, DataStore.set($scope.keyBase, selected)
                }, $scope.selected = function (subItem) {
                    return(selected || $scope.subtabs[0]) === subItem
                }, tabsetCtrl.addTab($scope)
            }, scope: {active: "=", disabled: "=", heading: "@"}}
        }
    }(), function () {
        RAML.Directives.subtabs = function () {
            return{restrict: "E", require: "^consoleTab", link: function ($scope, $element, $attrs, tabCtrl) {
        ...
        RAML.Directives.uriBar = function () {
            return{restrict: "E", require: "^consoleTab", link: function ($scope, $element, $attrs, tabCtrl) {
                $attrs.$observe("pathBuilder", function (pathBuilder) {
                    pathBuilder && tabCtrl.registerUriBar($scope)
                })
            }, scope: {pathBuilder: "=", baseUri: "=", pathSegments: "="}}
        }
    }()
}(), function () {
    "use strict";
    RAML.Directives.toggle = function () {
        return{restrict: "E", replace: !0, transclude: !0, controller: RAML.Controllers.toggle, templateUrl: "views/toggle.tmpl.html", scope: {keyBase: "@", toggleModel: "=?"}}
    }
}(), function () {
    "use strict";
    ...
    RAML.Directives.toggleItem = function () {
        return{restrict: "E", require: "^toggle", replace: !0, transclude: !0, link: link, templateUrl: "views/toggle-item.tmpl.html", scope: {heading: "@", active: "=?"}}
    }
}(), function () {
    "use strict";
    RAML.Directives.tryIt = function (DataStore) {
        return{restrict: "E", templateUrl: "views/try_it.tmpl.html", replace: !0, link: function ($scope) {
    ...
    RAML.Directives.validatedInput = function ($parse) {
        ...
        Controller.prototype.validate = function (scope, value) {
            var constraints = this.constraints(scope), validator = RAML.Client.Validator.from(constraints);
        ...
        var link = function ($scope, $el, $attrs, controllers) {
            function validateField() {
                ...
                errors ? $el.addClass(errorClass) : $el.removeClass(errorClass)
            }

            var modelController = controllers[0], validateController = controllers[1], errorClass = $parse($attrs.invalidClass)($scope) || "warning";
        ...
        return{restrict: "A", require: ["ngModel", "validatedInput"], controller: Controller, link: link}
    }
}(), RAML.Filters = {}, function () {
    "use strict";
    RAML.Filters.nameFromParameterizable = function () {
        return function (input) {
            return"object" == typeof input && null !== input ? Object.keys(input)[0] : input ? input : void 0
        }
    }
}(), function () {
    "use strict";
    ...
    RAML.Services.RAMLParserWrapper = function ($rootScope, ramlParser, $q) {
        var ramlProcessor, errorProcessor, whenParsed, PARSE_SUCCESS = "event:raml-parsed", load = function (file) {
            setPromise(ramlParser.loadFile(file))
        }, parse = function (raml) {
            setPromise(ramlParser.load(raml))
        }, onParseSuccess = function (cb) {
            ramlProcessor = function () {
                cb.apply(this, arguments), $rootScope.$$phase || $rootScope.$digest()
            }, whenParsed && whenParsed.then(ramlProcessor)
        }, onParseError = function (cb) {
            errorProcessor = function () {
                cb.apply(this, arguments), $rootScope.$$phase || $rootScope.$digest()
            }, whenParsed && whenParsed.then(void 0, errorProcessor)
        }, setPromise = function (promise) {
            whenParsed = promise, (ramlProcessor || errorProcessor) && whenParsed.then(ramlProcessor, errorProcessor)
        };
        return $rootScope.$on(PARSE_SUCCESS, function (e, raml) {
            setPromise($q.when(raml))
        }), {load: load, parse: parse, onParseSuccess: onParseSuccess, onParseError: onParseError}
    }
}(), function () {
    RAML.Settings = RAML.Settings || {};
    var location = window.location, uri = location.protocol + "//" + location.host + location.pathname + "authentication/oauth2.html";
    RAML.Settings.oauth2RedirectUri = RAML.Settings.oauth2RedirectUri || uri, RAML.Settings.oauth1RedirectUri = RAML.Settings.oauth1RedirectUri || uri.replace(/oauth2\.html$/, "oauth1.html")
}(), function () {
    "use strict";
    function Clone() {
    }

    RAML.Utils = {clone: function (object) {
        return Clone.prototype = object, new Clone
    }, copy: function (object) {
        var copiedObject = {};
        for (var key in object)copiedObject[key] = object[key];
        return copiedObject
    }, isEmpty: function (object) {
        return object ? 0 === Object.keys(object).length : !0
    }}
}(), function () {
