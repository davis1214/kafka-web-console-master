...
object Message {
  ...
  case class FetchBrokerMonitorInfo()

  case class FetchTopicMonitorInfo()

  case class FetchAlarmMonitorInfo()

  object ConnectNotification {
    def apply(zk: Zookeeper, status: Status.Value): ConnectNotification = {
      ConnectNotification(Zookeeper(zk.name, zk.host, zk.port, zk.groupId, status.id, zk.chroot, zk.topicConfig, zk.id))
    }
  }
