  ...
  implicit object ZookeeperWrites extends Writes[Zookeeper] {
    def writes(zookeeper: Zookeeper) = {
      ...
      Json.obj(
        ...
        "topicConfig" -> zookeeper.topicConfig
      )
    }
  }
...
case class Zookeeper(name: String, host: String, port: Int, groupId: Long, statusId: Long, chroot: String,
                     topicConfig: String, id: Long = 0)
  extends KeyedEntity[Long] {

  override def toString = "%s:%s/%s".format(host, port, chroot)

  def this(name: String, host: String, port: Int, groupId: Long, statusId: Long, chroot: String, id: Long = 0) {
    this(name, host, port, groupId, statusId, chroot, "", id)
  }
