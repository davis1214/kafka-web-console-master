...
object Topic extends Controller {
  ...
  def index_new = Action.async {
    val topicsZks = connectedZookeepers {
      (zk, zkClient) =>
        for {
  ...
  def index = Action {
    Ok(Json.toJson(models.TopicHistoryInfo.findCurrentTimeTopicInfo(1)))
  ...
  def feed(topic: String, zookeeper: String) = WebSocket.using[String] {
    implicit request =>
      ...
      val out = Concurrent.unicast[String] {
        channel: Concurrent.Channel[String] =>

          val cb = (messageHolder: MessageAndMetadata[String, String]) => {
            channel.push(messageHolder.message)
          }

          getZChildren(zkClient, "/brokers/topics/" + topic + "/partitions/*").map {
            p =>
  ...
  private def createTopicInfo(consumersAndPartitionOffsets: Map[String, Seq[Long]], partitionsLogSize: Seq[Long],
    ...
    consumersAndPartitionOffsets.map {
      cPO =>
